...
def _search_sql_cache(question: str) -> Optional[str]:
    if not SQL_CACHE:
    ...
    try:
        ...
        logger.info("Cache HIT (similarity=%.4f): %s", similarity, sql)
        return sql
...
def _store_sql_cache(question: str, sql: str):
    if not SQL_CACHE:
    ...
    try:
        ...
        logger.info("SQL guardado en cache exitosamente")
    except Exception:
        logger.exception("Error guardando SQL en cache")


# FILTRO DE INFO INNECESARIA
...
async def run_agent_stream_text(
    ...
    # 1. Intentar responder desde SQL cache
    cached_sql = _search_sql_cache(original_question)
    if cached_sql and SQL_RUNNER:
        try:
            tool_args = RunSqlToolArgs(sql=cached_sql)
            df = await SQL_RUNNER.run_sql(tool_args, None)
            logger.info("Cache HIT ejecutado correctamente (%d filas)", len(df))
            if not df.empty:
    ...
    def on_sql_executed(sql: str):
        if sql not in captured_sql:  # evitar duplicados si ambos disparan
    ...
    # 4. Ejecutar el agente y hacer stream de texto
    try:
    ...
    # 5. Guardar en cache si se capturó SQL exitosamente
    if captured_sql:
        executed_sql = captured_sql[-1]  # Usar el último SQL ejecutado
        logger.info("Guardando SQL en cache para: '%s'", original_question)
        _store_sql_cache(original_question, executed_sql)
    else:
